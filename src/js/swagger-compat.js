...
SwaggerResource.prototype.addModels = function (models) {
  if (typeof models === 'object') {
    ...
    for (modelName in models) {
      if (typeof this.models[modelName] === 'undefined') {
        var swaggerModel = new SwaggerModel(modelName, models[modelName], this);
...
var SwaggerModel = function (modelName, obj, resource) {
...
SwaggerModel.prototype.getMockSignature = function (modelsToIgnore) {
  ...
  for (i = 0; i < this.properties.length; i++) {
    ...
    if(this.discriminator && this.properties[i].name!=this.discriminator)
      continue;
  ...
  for (i = 0; i < this.properties.length; i++) {
    ...
    if ((prop.refModel) && modelsToIgnore.indexOf(prop.refModel.name) === -1) {
      ...
      prop.refModel.subTypes.forEach(function(subType) {
        if(self.resource.models[subType] && modelsToIgnore.indexOf(subType) === -1)
          returnVal = returnVal + ('<br>' + (self.resource.models[subType]).getMockSignature(modelsToIgnore));
      });
    }
  }
  return returnVal;
...
SwaggerModel.prototype.createJSONSample = function (modelsToIgnore) {
  if (sampleModels[this.name]) {
    ...
    this.subTypes.forEach(function(subType) {
      if(self.resource.models[subType])
        allProperties = allProperties.concat(self.resource.models[subType].properties);
    });
